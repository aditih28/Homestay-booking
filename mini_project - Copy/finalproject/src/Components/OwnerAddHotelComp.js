...
import { dynamicValue } from './LoginComp.js';        
    ...
    export default function OwnerAddHotelComp() {
      ...
      const hotelsData = {
        ...
        city: hotels.city.value,
        state: hotels.state.value,
       ...
       <h3 className="text-center mb-4 text-info">Hotel Registration</h3>
        ...
        <div className="mb-3">
          ...
          <div className="mb-3">
            ...
            <input
              type="text"
              className={`form-control ${hotels.state.touched && !hotels.state.valid ? 'is-invalid' : ''}`}
              value={hotels.state.value}
              onChange={(e) => handleChange('state', e.target.value)}
              onBlur={(e) => handleChange('state', e.target.value)}
            />
            {hotels.state.touched && !hotels.state.valid && (
              <div className="invalid-feedback">{hotels.state.error}</div>
            )}
          </div>
          ...
          <div className="mb-3">
            ...
            <input
              type="text"
              className={`form-control ${hotels.city.touched && !hotels.city.valid ? 'is-invalid' : ''}`}
              value={hotels.city.value}
              onChange={(e) => handleChange('city', e.target.value)}
              onBlur={(e) => handleChange('city', e.target.value)}
            />
            {hotels.city.touched && !hotels.city.valid && (
              <div className="invalid-feedback">{hotels.city.error}</div>
            )}
          </div>
